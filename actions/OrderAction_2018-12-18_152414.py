...
class orderAction(object):
    ...
    def _api_trigger_call_order_close_over_time(self, platForm, orderNo):
        data = {'_tk_': self.order.token, '_deviceId_': self.order.device_id, 'platForm': platForm, 'orderNo': orderNo}
        response = self.request.post(url='http://dev.ms.order.sjnc.com/api/trigger/call/order/close-over-time', data=data)
        json_response = json.loads(response)
        if json_response["status"] == "OK":
            pass
        elif json_response["status"] == "ERROR":
    ...
    def _api_trigger_call_order_close_over_time_by_order_no(self, platForm, orderNo):
        data = {'_tk_': self.order.token, '_deviceId_': self.order.device_id, 'platForm': platForm, 'orderNo': orderNo}
        response = self.request.post(url='http://dev.ms.order.sjnc.com/api/trigger/call/order/close-over-time/by-order-no', data=data)
        json_response = json.loads(response)
        if json_response["status"] == "OK":
            pass
        elif json_response["status"] == "ERROR":
    ...
    def _mobile_channel_order_list(self, orderStatus, pn, ps):
