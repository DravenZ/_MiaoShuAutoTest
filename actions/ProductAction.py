...
class productAction(object):
    ...
    def _mobile_product_nearby_shop_products(self, lng, lat, distance, categoryId, search, pn, ps):
        data = {'_tk_': self.product.token, '_deviceId_': self.product.device_id, 'lng': lng, 'lat': lat, 'distance': distance, 'categoryId': categoryId, 'search': search, 'pn': pn, 'ps': ps}
        response = self.request.post(url='http://dev.ms.product.sjnc.com/mobile/product/nearby-shop-products', data=data)
        json_response = json.loads(response)
        if json_response["status"] == "OK":
            pass
        elif json_response["status"] == "ERROR":
